...
protected:
    ...
    void transverseInOrder(int index, std::vector<std::string> &result);
    void transversePostOrder(int index, std::vector<std::string> &result);
    void transversePreOrder(int index, std::vector<std::string> &result);

    static void writePlayerToFile(const std::string &name, std::ofstream &outFile);

public:
    std::map<std::string, std::string> currentIsles;
    bool playerDeleted = false;
    std::string deletedPlayersIsle;
