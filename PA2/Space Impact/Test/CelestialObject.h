...
public:
    ...
    CelestialObject(const vector<vector<bool> > &shape, ObjectType type, int start_row, int time_of_appearance, CelestialObject* next, int occupiedCellCount);
    ...
    bool is_destroyed();
    void define_rotations();
    CelestialObject *rotate_right(int rowCount, int colCount, const vector<vector<bool>> &shape);
    CelestialObject *rotate_left(int rowCount, int colCount, const vector<vector<bool> > &shape) ;

private:

    bool is_symmetric(int rowCount, int colCount, const vector<vector<bool> > &shape);
