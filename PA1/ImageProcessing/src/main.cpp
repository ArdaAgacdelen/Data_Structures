...
void apply_mean_filter(const char *input_image, int kernel_size) {
    ...
    std::string output_filename = "mean_filtered_" + remove_extension(input_image) + "_" + std::to_string(kernel_size) +
                                  ".png";
...
void apply_gaussian_smoothing(const char *input_image, int kernel_size, double sigma) {
    ...
    std::string output_filename = "gaussian_filtered_" + remove_extension(input_image) + "_" +
                                  std::to_string(kernel_size) + "_" + std::to_string(sigma) + ".png";
    img.save_to_file(output_filename.c_str());
}
...
void apply_unsharp_mask(const char *input_image, int kernel_size, double amount) {
    ...
    std::string output_filename = "unsharp_filtered_" + remove_extension(input_image) + "_" +
                                  std::to_string(kernel_size) + "_" + std::to_string(amount) + ".png";
    img.save_to_file(output_filename.c_str());
}
